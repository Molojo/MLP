...
def plot_losses(train_losses, val_losses):
    """
    Plots the training and validation loss curves with additional embellishments.
    """
    ...
    plt.plot(epoch_range, train_losses, 'o-', color='blue', label='Training Loss', 
             markersize=5, linewidth=2)
    
    # Plot validation loss with square markers.
    plt.plot(epoch_range, val_losses, 's-', color='red', label='Validation Loss', 
             markersize=5, linewidth=2)
    
    # Add grid lines for readability.
    plt.grid(True, linestyle='--', alpha=0.7)
    
    # Annotate the best (minimum) validation loss.
    ...
    # Annotate the best (minimum) training loss.
    ...
    plt.annotate(
        ...
        xytext=(best_train_epoch - 3, best_train_loss + 0.05),
        arrowprops=dict(facecolor='black', shrink=0.05, width=1, headwidth=8),
        fontsize=12,
